...
class BJAgent(ABC):
    ...
    def __init__(
        ...
        self.env = gym.make(
            "Blackjack-v1", natural=natural, sab=sab, render_mode=render_mode
        )
        self.gamma = gamma
    ...
    def play(
        self, episodes=1_000, render_mode="human", print_results=True, epsilon=None
    ):
        env = gym.make(
            "Blackjack-v1", natural=self.natural, sab=self.sab, render_mode=render_mode
        )
        win = 0
        lose = 0
    ...
    def plot_history(self, return_fig=False):
        ...
        sns.lineplot(
            ...
            y=curve_fit,
            color="blue",
            label=f"Fitted curve (asymptote: {asymptote:.3f})",
            linestyle="-",
            linewidth=1.5,
            ax=ax,
        )
    ...
    def plot_policy(self, return_fig=False):
        ...
        player_count = range(4, 22)
        dealer_count = range(1, 11)
        usable_ace = [0, 1]
    ...
    def epsilon_update(
        self, episodes, validate_each_episodes, final_epsilon, epsilon_decay
    ):
    ...
    def validation(self, episode, episodes, epsilon_val, verbose, save):
        ...
        result = self.play(
            episodes=1_000, render_mode=None, print_results=False, epsilon=_epsilon_val
        )
        self.history.append(result)

        if verbose:
            print(
                f"Episode: {episode:7d}/{episodes}, epsilon: {self.epsilon:.5f}, Q_delta: {self.delta:.2e}, Win rate: {result[0]/(result[0]+result[1]):.3f}"
            )

        if save:
            save_agent(self, f"./models/{self.name}.pickle", verbose=verbose)
